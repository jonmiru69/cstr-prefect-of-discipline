...
export function Brand({compact=false, collapsed=false}:{compact?:boolean; collapsed?:boolean}){
  return (
    <div className={'brand ' + (compact ? 'compact ' : '') + (collapsed ? 'collapsed' : '')}>
      <img src="/cstr-logo.png" alt="CSTR Seal" className="brand-logo" />
      {!collapsed && (
  ...
  return (
    <main className="auth-page">
      <section className="auth-intro">
        <div className="auth-intro-overlay" />
        <div className="auth-intro-content">
          <div className="auth-brand-wrapper">
            <img src="/cstr-logo.png" alt="CSTR Seal" className="auth-school-logo" />
            <div className="auth-school-names">
              <span className="auth-school-pre">COLEGIO DE STO. TOMAS – RECOLETOS</span>
              <span className="auth-school-sub">San Carlos City, Negros Occidental · Founded 1940</span>
            </div>
          </div>
          <div className="auth-message">
            <div className="motto-badge"><span>CARITAS ET SCIENTIA</span></div>
            <p className="eyebrow gold">OFFICE OF THE PREFECT OF DISCIPLINE</p>
            <h1>Careful records.<br/>Accountable decisions.</h1>
            <p>A secure, confidential management portal for student discipline, case tracking, and administrative due process.</p>
            <div className="auth-rule" />
            <div className="auth-division-tag">
              <ShieldCheck size={18}/>
              <span>JUNIOR HIGH SCHOOL & SENIOR HIGH SCHOOL</span>
            </div>
          </div>
          <p className="auth-foot">Official School Portal · Authorized School Personnel Only</p>
        </div>
      </section>
      <section className="auth-form-panel">
        {children}
        <p className="privacy-contact">
          {import.meta.env.VITE_PRIVACY_CONTACT||'Contact the CSTR school office for privacy requests.'}{' '}
  ...
  if(!configured)return (
    <AuthLayout>
      <div className="auth-card">
        ...
        <h2>Connect the school portal</h2>
        <p>The website files are ready. The project owner must add the Supabase URL and publishable key in Cloudflare Pages, then redeploy.</p>
        <div className="setup-keys">
          <code>VITE_SUPABASE_URL</code>
          <code>VITE_SUPABASE_PUBLISHABLE_KEY</code>
        </div>
        <p className="subtle">Follow the included START-HERE guide. Student records remain empty until the office records a case.</p>
      </div>
    </AuthLayout>
  );
    ...
    <AuthLayout>
      <div className="auth-card">
        {loading?<Loading/>:!session?<>
          ...
          <p>Sign in with the Google account approved by the school.</p>
          <button className="button google" onClick={async()=>{
          ...
          <div className="auth-step-list">
            <span>01 <strong>Google account</strong></span>
            <span>02 <strong>School access</strong></span>
            <span>03 <strong>Authenticator</strong></span>
          </div>
          <p className="subtle small">First visit? Have your personal school access code ready.</p>
        </>:
        access?.state==='disabled'?<>
          <h2>Access is disabled</h2>
          <p>Contact the principal or system administrator to review your access.</p>
        </>:
        ...
        access?.state==='member'?<>
          {welcome&&<Alert success>{welcome}</Alert>}
          <MfaForm onSuccess={()=>setRefresh(v=>v+1)}/>
        </>:null}
...
function InviteForm({onSuccess}:{onSuccess:(name:string,role:Role)=>void}){
  ...
  return (
    <form onSubmit={async e=>{
      ...
      try{
        const r=await rpc<{ok:boolean;message?:string;name:string;role:Role}>('redeem_invitation',{p_code:code});
        if(!r.ok)throw new Error(r.message);
        setCode('');onSuccess(r.name,r.role);
      }catch(e){setError(errorMessage(e))}finally{setBusy(false)}
    }}>
      <KeyRound className="auth-icon"/>
      <p className="eyebrow">SCHOOL ACCESS</p>
      <h2>Your personal invitation</h2>
      <p>Paste the access code issued to your Google account. It can be used once.</p>
      <Field label="School access code">
        <input required type="password" autoComplete="off" maxLength={100} value={code} onChange={e=>setCode(e.target.value)} placeholder="Paste your personal code"/>
      </Field>
      {error&&<Alert>{error}</Alert>}
      <button className="button full" disabled={busy}>{busy?'Checking…':'Verify school access'}<ArrowRight size={18}/></button>
    </form>
  );
}

export function MfaForm({onSuccess,backup=false}:{onSuccess:()=>void;backup?:boolean}){
  const [choices,setChoices]=useState<{id:string;friendly_name?:string}[]>([]); 
  const [factor,setFactor]=useState(''),[qr,setQr]=useState(''),[secret,setSecret]=useState(''),[code,setCode]=useState('');
  ...
  useEffect(()=>{
    ...
    supabase!.auth.mfa.listFactors().then(({data,error})=>{
      ...
      setChoices(data.totp.filter(x=>x.status==='verified'));
      const f=data.totp.find(x=>x.status==='verified');
      if(f&&!backup)setFactor(f.id);else setEnroll(true);
      setLoading(false);
    });
    return()=>{live=false};
  },[backup]);
  ...
  return (
    <form onSubmit={async e=>{
      e.preventDefault();setBusy(true);setError('');
      try{
        const {error}=await supabase!.auth.mfa.challengeAndVerify({factorId:factor,code:code.replaceAll(' ','')});
        if(error)throw error;
        setSecret('');setQr('');setCode('');onSuccess();
      }catch(e){setError(errorMessage(e))}finally{setBusy(false)}
    }}>
      <ShieldCheck className="auth-icon"/>
      <p className="eyebrow">{backup?'BACKUP AUTHENTICATOR':'IDENTITY VERIFICATION'}</p>
      <h2>{enroll?'Set up your authenticator':'Enter your six-digit code'}</h2>
      ...
      {factor&&<>
        {!enroll&&choices.length>1&&<Field label="Authenticator device"><select value={factor} onChange={e=>setFactor(e.target.value)}>{choices.map((f,i)=><option key={f.id} value={f.id}>{f.friendly_name||'Authenticator '+(i+1)}</option>)}</select></Field>}
        <Field label="Authenticator code"><input required inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" minLength={6} maxLength={6} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,''))} placeholder="000000" className="otp"/></Field>
        <button className="button full" disabled={busy}>{busy?'Verifying…':'Verify and continue'}<ArrowRight size={18}/></button>
      </>}
      {error&&<Alert>{error}</Alert>}
    </form>
  );
